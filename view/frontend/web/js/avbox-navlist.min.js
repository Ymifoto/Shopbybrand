"use strict";class AlphaListNav{constructor(listElem,options={}){const defaultOptions={initHidden:!1,initLetter:'*',includeAll:!0,allText:'Összes',noMatchText:'No matching entries',includeNums:!0,concatenateNums:!0,includeOther:!1,flagDisabled:!0,removeDisabled:!0,prefixes:[],filterSelector:'',showCounts:!1,showLetterHeadings:!0,}
...
const ul=document.createElement('ul');ul.className='alpha-list-group row';alphaObj[key].forEach(node=>{ul.appendChild(node.cloneNode(!0))});div.appendChild(ul);wrapper.appendChild(div);return div});const noMatchDiv=document.createElement('div');noMatchDiv.id='no-match';const noMatchUl=document.createElement('ul');noMatchUl.className='no-match-group';const noMatchLi=document.createElement('li');noMatchLi.textContent=this.options.noMatchText;noMatchUl.appendChild(noMatchLi);noMatchDiv.appendChild(noMatchUl);wrapper.appendChild(noMatchDiv);return wrapper}
...
const listElem = document.getElementById('gyartok');
const gyartokList = new AlphaListNav(listElem);